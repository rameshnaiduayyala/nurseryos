...
import { Landmark, ArrowDownRight, ArrowUpRight, RefreshCw } from 'lucide-react';
import { api } from '../../services/api';
import { useAuth } from '../../context/AuthContext';

function LedgerTable({ title, subtitle, icon: Icon, entries, nameField, loading }) {
  if (loading) {
    return (
          ...
          <tbody>
            {entries.map((l) => (
              <tr key={l.id} className="border-b border-slate-50">
                ...
                <td className="py-3.5 font-semibold text-slate-800">{nameField(l)}</td>
...
export function CustomerLedgers() {
  ...
  return (
    <LedgerTable
      ...
      subtitle="Global debits & credits across all customers"
      icon={Landmark}
      entries={entries}
      loading={loading}
      nameField={(l) => l.customer?.name || 'Unknown Customer'}
    />
  );
}

export function FarmerLedgers() {
  const { setError } = useAuth();
  const [entries, setEntries] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const res = await api.reports.getFarmerLedger();
        setEntries(res.data || []);
      } catch (err) {
        setError(err.message);
      } finally {
  ...
  return (
    <LedgerTable
      title="Farmer Ledger"
      subtitle="Global payment history across all farmers"
      icon={Landmark}
      entries={entries}
      loading={loading}
      nameField={(l) => l.farmer?.fullName || 'Unknown Farmer'}
    />
  );
}
